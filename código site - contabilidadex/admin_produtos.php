...
function buscarProdutos($pdo, $termo_busca) {
    // Query para buscar produtos que correspondam ao termo de busca
    $query = "SELECT * FROM produtos WHERE nome LIKE :termo_busca";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['buscar'])) {
    ...
    $produtos = buscarProdutos($pdo, $termo_busca);
} else {
    // Exibir todos os produtos inicialmente
    $produtos = buscarProdutos($pdo, '');
}

// Processar a exclusão de produto
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['excluir'])) {
    $produto_id = $_POST['produto_id'];

    // Query para excluir produto
    ...
    header("Location: admin_produtos.php?message=" . urlencode($message));
    exit();
}
?>
...
<head>
    ...
    <title>Administrar Produtos - ContabilidadeX</title>
    <!-- Bootstrap CSS -->
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
    <div class="container mt-5">
        <h1 class="mb-4">Administrar Produtos</h1>

        <!-- Formulário de Busca -->
        <form action="admin_produtos.php" method="POST" class="form-inline mb-4">
            <div class="form-group mx-sm-3 mb-2">
                <label for="termo_busca" class="sr-only">Buscar Produto por Nome:</label>
                <input type="text" class="form-control" id="termo_busca" name="termo_busca" placeholder="Nome do Produto" value="<?php echo isset($termo_busca) ? $termo_busca : ''; ?>" required>
            </div>
            ...
            <ul class="list-group">
                <?php foreach ($produtos as $produto): ?>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        ...
                        <form action="admin_produtos.php" method="POST" style="display:inline;">
        ...
        <?php elseif (isset($produtos) && count($produtos) == 0): ?>
